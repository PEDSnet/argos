...
.env_cleanup <- function(db = config('db_src'))
  get_argos_default()$.env_cleanup(db)

argos$set(
  'private', '.env_cleanup',
  function(db = self$config('db_src')) {
    ...
    if (any(class(db) == 'OraConnection')) private$.ora_env_cleanup(db)
    else if (any(class(db) == 'PqConnection')) private$.pq_env_cleanup(db)
    else if (any(class(db) == 'PostgreSQLConnection')) private$.pgsql_env_cleanup(db)
...
.env_setup <- function(db = config('db_src'))
  get_argos_default()$.env_setup(db)
...
argos$set(
  ...
  function(db = self$config('db_src')) {
    ...
    if ( is.na(Sys.getenv('PEDSNET_SKIP_REQUEST_EXECUTION', unset = NA)) &&
         ! is.null(self$config('execution_mode')) &&
         self$config('execution_mode') == 'distribution' ) {
      logf <- file(file.path(self$config('base_dir'),
    ...
    if (packageVersion('dbplyr') < '1.3.1') {
      ...
      # Fix bug in 2.2.x that calls unname() indiscriminately and wipes out schema
    ...
    if (any(class(db) == 'OraConnection')) private$.ora_env_setup(db)
    else if (any(class(db) == 'PqConnection')) private$.pq_env_setup(db)
    else if (any(class(db) == 'PostgreSQLConnection')) private$.pgsql_env_setup(db)

    invisible(TRUE)
  })

.pq_env_setup <- function(db)
  get_argos_default()$.pq_env_setup(db)

argos$set(
  'private', '.pq_env_setup',
  function(db) {
    if (packageVersion('dbplyr') < '2.0.0') {
      assignInNamespace('db_has_table.DBIConnection',
                        function(con, table, ...) self$db_exists_table(con, table),
                        ns = 'dbplyr')
    }
  })

.pq_env_cleanup <- function(db) invisible(TRUE)

argos$set(
  'private', '.pq_env_cleanup',
  function(db) {
    invisible(TRUE)
  })


.pgsql_env_setup <- function(db)
  get_argos_default()$.pgsql_env_setup()

argos$set(
  'private', '.pgsql_env_setup',
  function(db) {
    if (packageVersion('dbplyr') < '2.0.0') {
...
.pgsql_env_cleanup <- function(db) invisible(TRUE)

argos$set(
  'private', '.pgsql_env_cleanup',
  function(db) {
    invisible(TRUE)
  })


.pq_env_cleanup <- function(argobj) invisible(TRUE)


.ora_env_setup <- function(db)
  get_argos_default()$.ora_env_setup(db)

argos$set(
  'private', '.ora_env_setup',
  function(db) {
    ...
    self$config('ora_drop_me', character(0))
    ...
    if (packageVersion('dbplyr') < '2.0.0') {
      ...
      assignInNamespace('db_create_index.DBIConnection',
                        function(con, tabname, columns, name = NULL,
                                 unique = FALSE, ...) {
                          assertthat::assert_that(is_string(tabname),
                          ...
                          fields <- dbplyr::escape(ident(columns), parens = TRUE,
                                                   con = con)
...
.ora_env_cleanup <- function(db)
  get_argos_default()$.ora_env_cleanup(db)

argos$set(
  'private', '.ora_env_cleanup',
  function(db) {
    dm <- self$config('ora_drop_me')
    if (length(dm) == 0) return(invisible(TRUE))

    vapply(unique(dm),
           function (t)
             tryCatch(self$db_remove_table(db, t, temporary = TRUE),
